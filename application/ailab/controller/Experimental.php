...
class Experimental  extends Controller
{
	...
	//接收实验数据并写入数据库
	public function ailabExperimentalData()
	{
		//更新使用次数
		$software = Db::name('ailab_software');
		$where = [
			'softwarename' => '无人机智能任务规划虚拟仿真实验',
		];
		$select = $software->where($where)->find();
		if($select){
			$updates = $software->where($where)->setInc('num',1);
		}else{
			$adds = [
				'softwarename' => '无人机智能任务规划虚拟仿真实验',
				'num' 		   => '1',
			];
			$updates = $software->insert($adds);
		}
		//更新使用次数结束
		...
		$processdata = Db::name('ailab_processdata');
